...
def _require_write_scope():
    ...
    if status["writable"] is False:
        raise CalendarAuthError(
            "Google Calendar write permission is not granted. Reconnect Google once with the calendar scope."
        )
...
def _iso(value, *, end_of_day=False):
    ...
    if isinstance(value, datetime):
        ...
        try:
            ...
            # Date-only values are interpreted in the app timezone.
            try:
...
def list_events(start=None, end=None, limit=250):
    ...
    # Dedupe defensively. This also prevents the repeated "Happy birthday!"
    # rows caused by malformed/duplicated payload handling in older builds.
    seen = set()
    events = []
...
def _event_body(data, existing=None):
    ...
    if "extended_properties" in data and isinstance(data.get("extended_properties"), dict):
        body["extendedProperties"] = {
            "private": {
                str(k): str(v)
                for k, v in (data.get("extended_properties") or {}).items()
                if v is not None
            }
        }
    ...
    if has_start:
        ...
        # Google expects start/end together for time changes, so preserve start.
...
def find_managed_event(marker, start=None, end=None):
    """Find an Agent Harness-created event without scanning the whole calendar."""
    if not marker:
        return None
...
def upsert_ai_deadline_event(marker, data):
    """
    Idempotently create/update a Google Calendar event generated from an
    email-derived deadline. The private marker prevents duplicates.
    """
    _require_write_scope()
    if not marker:
    ...
    # 5 = banana/yellow-ish, 6 = tangerine/orange-ish, 11 = tomato/red-ish.
    # UI still applies its own accessible classes; this only helps in Google Calendar.
